...
export async function importCommand(suri) {
  if (!suri || suri.length === 0) {
    throw new Error(
      "Missing SURI argument. Examples:\n" +
        '  crrp import "//Alice"\n' +
        '  crrp import "word1 word2 ... word12"\n' +
        '  crrp import "word1 word2 ... word12//MyAccount"\n' +
        "  crrp import 0x<64-hex-chars>",
    );
  }
  ...
  let mapped = "unknown";
  try {
    mapped = (await isAccountMapped(h160)) ? "mapped" : "NOT mapped — run `crrp map`";
  } catch (err) {
  ...
  process.stderr.write(
    "\nWARNING: the SURI is stored in plaintext at the path above. " +
      ...
      "CRRP_SIGNER_SURI environment variable instead of `crrp import`.\n",
  );
}
